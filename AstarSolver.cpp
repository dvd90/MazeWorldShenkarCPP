...
Maze2d AstarSolver::Solve(Maze2d data)
{
...
std::pair<int, int> AstarSolver::Up(const pair<int, int> node) const
{
    return std::make_pair(node.first, node.second - 1);
}

std::pair<int, int> AstarSolver::Right(const pair<int, int> node) const
{
    return std::make_pair(node.first + 1, node.second);
}

std::pair<int, int> AstarSolver::Left(const pair<int, int> node) const
{
    return std::make_pair(node.first - 1, node.second);
}

std::pair<int, int> AstarSolver::Down(const pair<int, int> node) const
{
    return std::make_pair(node.first, node.second + 1);
}

const double AstarSolver::CalculateDistance(pair<int, int> current_node) const
{
    return std::sqrt(std::pow(current_node.first - goal_.first, 2) +
                     std::pow(current_node.second - goal_.second, 2));
}

bool AstarSolver::IsNotObstacle(std::pair<int, int> node) const
{
    if (grid_[node.second][node.first].atr == Wall)
        return false;
    else
        return true;
}

bool AstarSolver::IsWithinRegion(std::pair<int, int> node) const
{
...
const int AstarSolver::TakeDecision1(const std::pair<int, int> &new_node, const double &cost_g, ListInfo &info,
...
const int AstarSolver::TakeDecision2(const std::pair<int, int> &new_node, const double &cost_g, ListInfo &info,
                                     const std::pair<int, int> &parent_node)
{
    ...
    else if (IsWithinRegion(new_node) && IsNotObstacle(new_node))
    {
        ...
        { // If key could not be found in open list, then do the following --
...
void AstarSolver::PlotTrajectory()
{
    int status = Action();
    if (status == 1)
    {
        ...
        while (parent != node)
        {
            ...
            {
                board[node.second][node.first].gen = Visited;
            }
...
void AstarSolver::setStart(Position pos)
{
    start_ = std::make_pair(pos.x, pos.y);
}

void AstarSolver::setGoal(Position pos)
{
    goal_ = std::make_pair(pos.x, pos.y);
}
