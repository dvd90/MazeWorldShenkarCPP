...
void Table::show_bfs_result()
{
    ...
    cout << "End point is ( " << final_p.x << " , " << final_p.y << " )\n";
}
...
Maze2d BFSSolver::Solve(Maze2d data)
{

    Table t;
    do
    {
        t.fill_random(data._Size);
        t.travers_and_print();
    } while (t.find_way_BFS() != true);
    t.show_bfs_result();
